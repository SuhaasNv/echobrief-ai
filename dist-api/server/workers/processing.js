...
export async function processMeeting(job) {
    const sql = getSql();
    // ---- 1. Transcribe ----------------------------------------------------
  ...
  `;
    ...
    if (analysis.action_items.length > 0) {
        await sql `
      INSERT INTO action_items ${sql(analysis.action_items.map((item) => ({
    ...
    if (chunks.length > 0) {
        ...
        await sql `
      INSERT INTO transcript_chunks ${sql(chunks.map((chunk, i) => ({
            meeting_id: job.meeting_id,
            user_id: job.user_id,
    ...
    try {
        ...
        const { score, cost_usd } = await scoreMeeting(result.raw_text, result.speakers, analysis.action_items.length);
...
export async function markFailed(job, err) {
    const sql = getSql();
...
async function notifyComplete(job) {
  ...
  `;
    ...
    if (!row?.email)
        return;
    await sendProcessingCompleteEmail(row.email, row.title, `${env.APP_URL}/app/meetings/${job.meeting_id}`);
}
