...
app.post("/:provider/connect", zValidator("param", z.object({ provider: IntegrationProvider })), async (c) => {
    ...
    const state = await encrypt(JSON.stringify({ user_id: user.id, ts: Date.now() }), env.INTEGRATION_TOKEN_ENCRYPTION_KEY);
...
app.get("/:provider/callback", zValidator("param", z.object({ provider: IntegrationProvider })), zValidator("query", OAuthCallbackQuery), async (c) => {
    const { provider } = c.req.valid("param");
    ...
    void code;
    const encryptedAccess = await encrypt(tokenResult.access_token, env.INTEGRATION_TOKEN_ENCRYPTION_KEY);
...
app.delete("/:provider", zValidator("param", z.object({ provider: IntegrationProvider })), async (c) => {
    const { provider } = c.req.valid("param");
    const user = c.get("user");
    const sql = getSql();
