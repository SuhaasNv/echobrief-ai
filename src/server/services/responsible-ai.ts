 ...
 */
export async function checkToxicity(input: string): Promise<{ isSafe: boolean; categories?: string[] }> {
  try {
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a factual consistency checker. Your job is to verify if the 'Claim' is strictly supported by the 'Context'. Answer ONLY 'YES' or 'NO'."
        },
        {
          role: "user",
