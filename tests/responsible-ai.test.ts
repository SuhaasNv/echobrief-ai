...
import { redactPII, checkPromptInjection, checkToxicity, checkHallucination } from '../src/server/services/responsible-ai';
...
describe('Responsible AI Pipeline Nodes', () => {
  ...
  describe('PII Redaction Node', () => {
    ...
    const phones = Array.from({ length: 100 }).map((_, i) => `555-${i.toString().padStart(3, '0')}-0000`);
